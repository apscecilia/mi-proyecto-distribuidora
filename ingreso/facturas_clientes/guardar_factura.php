...
if ($accion=="modificar") {
	...
	$sel_lineas = "SELECT codigo,cantidad FROM factulinea WHERE codfactura='$codfactura' order by numlinea";//,codfamilia
	...
	while ($contador < mysql_num_rows($rs_lineastmp)) {
		...
		$sel_insert = "INSERT INTO factulinea (codfactura,numlinea,codigo,cantidad,precio,importe,dcto) 
		VALUES ('$codfactura','','$codigo','$cantidad','$precio','$importe','$dcto')";
		$rs_insert = mysql_query($sel_insert);//,codfamilia  '$codfamilia',
		...
		$sel_bajominimo = "SELECT codarticulo,stock,stock_minimo,descripcion FROM articulos WHERE codarticulo='$codigo' AND borrado=0";//codfamilia,   AND codfamilia='$codfamilia'
