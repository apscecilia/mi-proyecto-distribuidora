...
if ($accion=="alta") {
	...
	if (mysql_num_rows($rs_comprobar) > 0 ) {
			...
			while ($contador < mysql_num_rows($rs_tmp)) {
				...
				$sel_insertar="INSERT INTO factulineap (codfactura,codproveedor,numlinea,codigo,cantidad,precio,importe,dcto) VALUES 
				('$codfactura','$codproveedor','$numlinea','$codigo','$cantidad','$precio','$importe','$dcto')";//codfamilia,'$codfamilia',
...
if ($accion=="modificar") {
	...
	$sel_lineas = "SELECT codigo,cantidad FROM factulineap WHERE codfactura='$codfactura' AND codproveedor='$codproveedor' order by numlinea";//codfamilia,
	...
	while ($contador < mysql_num_rows($rs_lineastmp)) {
		...
		$sel_insert = "INSERT INTO factulineap (codfactura,codproveedor,numlinea,codigo,cantidad,precio,importe,dcto) 
		VALUES ('$codfactura','$codproveedor','','$codigo','$cantidad','$precio','$importe','$dcto')";//codfamilia,  '$codfamilia',
	...
	$preciototal=0;
	$baseimponible=0;
	if ($rs_act) { $mensaje="Los datos de la factura sido modificados correctamente"; }
	$cabecera1="Inicio >> Compras &gt;&gt; Modificar Factura";
	$cabecera2="MODIFICAR FACTURA";
}
//DAMOS DE BAJA UNA FACTURA
if ($accion=="baja") {
	$codfactura=$_GET["codfactura"];
	$codproveedor=$_GET["codproveedor"];
	$query="UPDATE facturasp SET borrado=1 WHERE codfactura='$codfactura'";
		...
		<div id="pagina">
			<div id="zonaContenido">
				...
				<div id="frmBusqueda">
					...
					<table class="fuente8" width="98%" cellspacing=0 cellpadding=3 border=0 ID="Table1">
					  <? $sel_lineas="SELECT factulineap.*, articulos.referencia,articulos.descripcion FROM factulineap,articulos WHERE factulineap.codfactura='$codfactura' AND factulineap.codproveedor='$codproveedor' AND factulineap.codigo=articulos.codarticulo  ORDER BY factulineap.numlinea ASC";
