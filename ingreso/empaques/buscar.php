...
$query_busqueda="SELECT count(*) as filas FROM empaques WHERE borrado=0 AND ".$where;
...
<html>
	<head>
		...
		function ver_embalaje(codembalaje) {
			parent.location.href="ver_empaque.php?codembalaje=" + codembalaje + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
		
		function modificar_embalaje(codembalaje) {
			parent.location.href="modificar_empaque.php?codembalaje=" + codembalaje + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
		
		function eliminar_embalaje(codembalaje) {
			parent.location.href="eliminar_empaque.php?codembalaje=" + codembalaje + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
		...
		<div id="pagina">
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<? echo $filas?>">
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<? $sel_resultado="SELECT * FROM empaques WHERE borrado=0 AND ".$where;
					...
					<table class="fuente8" width="85%" cellspacing=0 cellpadding=3 border=0>
						<tr>
							<td width="100%" class="mensaje"><?php echo "No hay ning&uacute;n empaque que cumpla con los criterios de b&uacute;squeda";?></td>
					    </tr>
					</table>					
					<? } ?>					
				</div>
