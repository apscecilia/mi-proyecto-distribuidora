...
if ($modif<>1) {
		if (!isset($codfacturatmp)) { 
			...
			$retorno=1; }
		if ($retorno==0) {	
				$codart=$_POST["codart"];//codbarras
				$sel_articulos="SELECT * FROM articulos WHERE codarticulo='$codart'";//codigobarra='$codbarra'
				...
				$sel_insert="INSERT INTO factulineatmp (codfactura,numlinea,codigo,cantidad,precio,importe,dcto) VALUES ('$codfacturatmp','','$codarticulo','$cantidad','$precio','$importe','$descuento')";//codfamilia '$codfamilia',
...
<?php
$sel_lineas="SELECT factulineatmp.*,articulos.* FROM factulineatmp,articulos WHERE factulineatmp.codfactura='$codfacturatmp' AND factulineatmp.codigo=articulos.codarticulo ORDER BY factulineatmp.numlinea ASC";
/*
...
for ($i = 0; $i < mysql_num_rows($rs_lineas); $i++) {
	...
	if ($i % 2) { $fondolinea="itemParTabla"; } else { $fondolinea="itemImparTabla"; } ?>
			...
			<tr class="<? echo $fondolinea?>">
				<td width="4%"><? echo $i+1?></td>
				<!--<td width="20%"><? echo $nombrefamilia?></td>-->
				<td width="16%"><? echo $referencia?></td>
				<td width="29%"><? echo $descripcion?></td>
				<td width="9%" class="aCentro"><? echo $cantidad?></td>
				<td width="8%" class="aCentro"><? echo $precio?></td>
				<td width="7%" class="aCentro"><? echo $descuento?></td>
				<td width="6%" class="aCentro"><? echo number_format($importe,2,".",","); ?></td>
			...
			<table width="100%" border=0 align="center" cellpadding=3 cellspacing=0 class="fuente8">
			  ...
			  <tr>
				...
				<td align="right"><div align="right"><? echo $simbolomoneda ?>
			      <input class="cajaTotales" name="baseimpuestos" type="text" id="baseimpuestos" size="12" align="right" value="<? echo number_format($baseiva,2,".",",");?>" readonly> 
		        </div></td>
			  </tr>
			  <tr>
				<td class="busqueda"><b>Precio Total</b></td>
				<td align="right"><div align="right"><? echo $simbolomoneda ?>
			      <input class="cajaTotales" name="preciototal" type="text" id="preciototal" size="12" align="right" value="<? echo number_format($preciotot,2,".",",");?>" readonly> 
		        </div></td>
			  </tr>
		</table>
			  </div>
