...
function pon_prefijo(nombre,cuil) {
	parent.document.formulario.nombre.value=nombre;
	parent.document.formulario.cuil.value=cuil;
}

function limpiar() {
	parent.document.formulario.nombre.value="";
	parent.document.formulario.cuil.value="";
...
<?
	...
	if (mysql_num_rows($rs_tabla)>0) {
		...
		pon_prefijo("<? echo mysql_result($rs_tabla,0,nombre) ?>","<? echo mysql_result($rs_tabla,0,cuil) ?>");
		</script>
		<? 
	} else { ?>
	<script>
