...
if ($accion=="alta") {
	$query_operacion="INSERT INTO remitos(codalbaran, codfactura, fecha, iva, codcliente, estado, borrado) VALUES ('', '0', '$fecha', '$iva', '$codcliente', '1', '0')";					
	$rs_operacion=mysql_query($query_operacion);
	$codalbaran=mysql_insert_id();
	if ($rs_operacion) { $mensaje="El remito ha sido dado de alta correctamente"; }
	$query_tmp="SELECT * FROM remilineatmp WHERE codalbaran='$codalbarantmp' ORDER BY numlinea ASC";
	...
	while ($contador < mysql_num_rows($rs_tmp)) {
		...
		$sel_insertar="INSERT INTO remilinea (codalbaran,numlinea,codigo,cantidad,precio,importe,dcto) VALUES 
		('$codalbaran','$numlinea','$codigo','$cantidad','$precio','$importe','$dcto')";//,'$codfamilia',
	...
	$sel_act="UPDATE remitos SET totalalbaran='$preciototal' WHERE codalbaran='$codalbaran'";
	$rs_act=mysql_query($sel_act);
	$baseimponible=0;
	$preciototal=0;
	$baseimpuestos=0;
	$cabecera1="Inicio >> Ventas &gt;&gt; Nuevo Remito ";
	$cabecera2="INSERTAR REMITO";
}

if ($accion=="modificar") {
	$codalbaran=$_POST["codalbaran"];
	$act_albaran="UPDATE remito SET codcliente='$codcliente', fecha='$fecha', iva='$iva' WHERE codalbaran='$codalbaran'";
	$rs_albaran=mysql_query($act_albaran);
	$sel_lineas = "SELECT codigo,cantidad FROM remilinea WHERE codalbaran='$codalbaran' order by numlinea";//codfamilia,
	...
	$sel_borrar = "DELETE FROM remilinea WHERE codalbaran='$codalbaran'";
	$rs_borrar = mysql_query($sel_borrar);
	$sel_lineastmp = "SELECT * FROM remilineatmp WHERE codalbaran='$codalbarantmp' ORDER BY numlinea";
	...
	while ($contador < mysql_num_rows($rs_lineastmp)) {
		...
		$sel_insert = "INSERT INTO remilinea (codalbaran,numlinea,codigo,,cantidad,precio,importe,dcto) 
		VALUES ('$codalbaran','','$codigo','$cantidad','$precio','$importe','$dcto')";//codfamilia' $codfamilia',  
	...
	$preciototal=0;
	$baseimpuestos=0;
	if ($rs_act) { $mensaje="Los datos del remito han sido modificados correctamente"; }
	$cabecera1="Inicio >> Ventas &gt;&gt; Modificar Remito ";
	$cabecera2="MODIFICAR REMITO ";
}

if ($accion=="baja") {
	$codalbaran=$_GET["codalbaran"];
	$query="UPDATE remitos SET borrado=1 WHERE codalbaran='$codalbaran'";
	$rs_query=mysql_query($query);
	$query="SELECT * FROM remilinea WHERE codalbaran='$codalbaran' ORDER BY numlinea ASC";
	...
	if ($rs_query) { $mensaje="El remito ha sido eliminado correctamente"; }
	$cabecera1="Inicio >> Ventas &gt;&gt; Eliminar remito ";
	$cabecera2="ELIMINAR REMITO";
	$query_mostrar="SELECT * FROM remitoS WHERE codalbaran='$codalbaran'";
...
if ($accion=="convertir") {
	...
	$fecha=$_POST["fecha"];
	$fecha=explota($fecha);
	$sel_albaran="SELECT * FROM remitoS  WHERE codalbaran='$codalbaran'";
	...
	$act_albaran="UPDATE remitos  SET codfactura='$codfactura',estado='2' WHERE codalbaran='$codalbaran'";
	$rs_act=mysql_query($act_albaran);
	$sel_lineas="SELECT * FROM remilinea WHERE codalbaran='$codalbaran' ORDER BY numlinea ASC";
	$rs_lineas=mysql_query($sel_lineas);
	$contador=0;
	while ($contador < mysql_num_rows($rs_lineas)) {
		//$codfamilia=mysql_result($rs_lineas,$contador,"codfamilia");
		...
		$sel_insert="INSERT INTO factulinea (codfactura,numlinea,codigo,cantidad,precio,importe,dcto) VALUES 
			('$codfactura','','$codigo','$cantidad','$precio','$importe','$dcto')";//codfamilia, '$codfamilia',
		$rs_insert=mysql_query($sel_insert);
		$contador++;
	}
	$mensaje="El remito ha sido convertido correctamente";
	$cabecera1="Inicio >> Ventas &gt;&gt; Convertir Remito";
	$cabecera2="CONVERTIR REMITO";
}
...
<html>
	<head>
		...
		function imprimir(codalbaran) {
			window.open("../fpdf/imprimir_remito.php?codalbaran="+codalbaran);
		}
		...
		<div id="pagina">
			<div id="zonaContenido">
				...
				<div id="frmBusqueda">
					...
					<table class="fuente8" width="98%" cellspacing=0 cellpadding=3 border=0 ID="Table1">
					  <? $sel_lineas="SELECT remilinea.*,articulos.* FROM remilinea,articulos WHERE remilinea.codalbaran='$codalbaran' AND remilinea.codigo=articulos.codarticulo ORDER BY remilinea.numlinea ASC";
$rs_lineas=mysql_query($sel_lineas);
