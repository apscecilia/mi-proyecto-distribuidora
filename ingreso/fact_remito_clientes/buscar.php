...
$numremitoini=$_POST["numremitoini"];//$numalbaranini
//$numremitofin=$_POST["numremitofin"];//$numalbaranfin
...
if ($numremitoini == "") { $numremitoini=0; } else { $where.=" AND codalbaran>='$numremitoini'"; }// albaran.$numalbaranini
//if ($numremitofin == "") { $numremitofin=9999999999; } else { $where.=" AND codalbaran<='$numremitofin'"; }
...
$query_busqueda="SELECT count(*) as filas FROM remitos,clientes WHERE remitos.estado=1 AND remitos.borrado=0 AND remitos.codcliente=clientes.codcliente AND ".$where;//albaranes
		...
		<div id="pagina">
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<? echo $filas?>">
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<? $sel_resultado="SELECT codalbaran,clientes.nombre as nombre,remitos.fecha as fecha,totalalbaran FROM remitos,clientes WHERE remitos.estado=1 AND remitos.borrado=0 AND remitos.codcliente=clientes.codcliente AND ".$where;//albaranes
