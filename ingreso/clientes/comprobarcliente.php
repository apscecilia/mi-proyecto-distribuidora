...
function pon_prefijo(nombre) {
	...
	parent.document.form_busqueda.cuil.value=cuil;
}//,nif

function limpiar() {
	parent.document.form_busqueda.nombre.value="";
	parent.document.form_busqueda.cuil.value="";
...
<?
	...
	if (mysql_num_rows($rs_tabla)>0) {
		...
		pon_prefijo("<? echo mysql_result($rs_tabla,0,nombre,cuil) ?>","<? echo mysql_result($rs_tabla,0) ?>");//,nif
		</script>
		<? 
	} else { ?>
	<script>
